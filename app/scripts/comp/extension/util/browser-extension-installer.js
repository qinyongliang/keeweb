...
function getWindowsManifestFileName(browser, extension) {
    ...
    return path.join(window.utools.getPath('userData'), manifestName);
}
...
function getNativeMessagingHostPath() {
    ...
    switch (process.platform) {
        case 'darwin':
            return path.join(window.utools.getPath('exe'), '..', 'util', 'keeweb-native-messaging-host');
        case 'win32':
            return path.join(window.utools.getPath('exe'), '..', 'keeweb-native-messaging-host.exe');
        case 'linux':
            return path.join(window.utools.getPath('exe'), '..', 'keeweb-native-messaging-host');
    }
}

async function install(browser, extension) {
...
async function uninstall(browser, extension) {
    if (process.platform === 'win32') {
        const registryPath = getWindowsRegistryPath(browser);
        if (!registryPath) {
            return;
